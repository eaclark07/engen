...
// Author: Yangfeng Ji
// Date: 09-28-2016
// Time-stamp: <yangfeng 10/24/2017 11:51:39>
...
int main(int argc, char** argv) {
  ...
  if (task == "train"){
	...
	if (model_type == "gen"){
	  eloss = smm.BuildGraph(doc, cg, d, 0, entityweight, false);
	...
	for (auto& doc : dev) {
	  ...
	  if (model_type == "gen"){
	    dev_eloss = smm.BuildGraph(doc, cg, d, 0, entityweight);
	  } else if (model_type == "dis"){
    ...
    for (auto& doc : tst){
      ...
      smm.InitGraph(cg, 0.0);
      if (model_type == "gen"){
	tst_eloss = smm.BuildGraph(doc, cg, d, evalobj, entityweight);
      } else if (model_type == "dis"){
	...
	ComputationGraph cg;
	smm.InitGraph(cg, 0.0);
	Expression eloss = smm.BuildGraph(doc, cg, d, 0, entityweight, true);
	string sampledtext = smm.get_sampledtext();
	cg.forward(eloss);
#if NODEBUG
	LOG(INFO) << "Sampled text = " << sampledtext
		  << "=== " << doc.filename << " " << doc.didx;
#else
	cerr << "Sampled text = " << sampledtext
